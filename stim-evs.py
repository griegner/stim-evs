...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="input thermal stimulator recordings, outputs event timings for each heat plateau"
    )
    parser.add_argument(
        "path", type=Path, help="path to TCS temperature recording (csv file)"
    )
    parser.add_argument("sub", type=str, help="subject ID (ex c000)")
    parser.add_argument("ses", type=str, help="scanning session (ex mri1,mri2..)")
    parser.add_argument("task", type=str, help="manipulation-stimuli (ex pre48C)")
    parser.add_argument(
        "--adjust_onsets",
        type=float,
        default=0,
        help="seconds to subtract from onset times",
    )
...
def plot(df, active, deriv, evs, axs):
    ...
    active.plot(ax=axs[0], c="k", lw=0.5)
    axs[0].plot(evs.index, evs.values, "ro", ms=2)
    deriv.plot(ax=axs[1], c="k", lw=0.5)
    ...
    axs[0].set_ylabel("°C")
    axs[0].axvline(x=0, color="r", linestyle=":", lw=0.6)
...
def evs_to_bids(evs):
    return pd.DataFrame(
        {"onset": evs.index[::2], "duration": evs.index[1::2] - evs.index[::2]}
    )
...
def main():
    ...
    fig, axs = plt.subplots(
        2, figsize=(30, 2), sharex=True, gridspec_kw={"height_ratios": [2, 1]}
    )
